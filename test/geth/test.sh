...
bootSecretFile="./bootnode/boot-secret"
bootAddress=$(<"./bootnode/boot-address")
...
function initGethNodes {
    ...
    do
        ...
        cp -r "./keystore" "./datadirs/data_$i"
    done
    rm genesis.json
}
...
function startBootNode {
    ...
    bootSecretKey=$(<"$1" )
    bootnode -nodekey "./bootnode/boot-secret" -addr ":30299"> "datadirs/bootnode.log" 2>&1 &
}
...
function startNode {
    ...
    geth --datadir "$datadir" --unlock "$coinbase" --password "./password" --rpc --rpcport "$rpcPort" --networkid 15 --port "$port" --mine --minerthreads=1 --etherbase="$coinbase" --verbosity 11 --bootnodes "$bootnodeAddr" >> "$datadir/logs/geth.log" 2>&1 &
    gethPIDs[$1]="$!"
}
function startNodes {
    for i in {0..2}
...
function main {
  ...
  runScriptOnNode "./js/createSimpleStorage.js" 0

  # run a method call script on node 2 20 times to build some old state
  addr=$(<"./contracts/address0")
  for i in {1..10}
  do
      runScriptOnNode "./js/callSetSimpleStorage.js" 2 $addr $i 
  done
  ...
  runScriptOnNode "./js/selectBlock.js" 0 10

  # take node 0 down 
  stopNode 0

  # run a contracts creation script on node 1
  runScriptOnNode "./js/createSimpleStorage.js" 1

  # run a method call script on node 2
  addr=$(<"./contracts/address1")

  runScriptOnNode "./js/callSetSimpleStorage.js" 2 $addr 3
  ...
  runScriptOnNode "./js/callSetSimpleStorage.js" 0 $addr 1

  addr=$(<"./contracts/address0")
  runScriptOnNode "./js/callSetSimpleStorage.js" 0 $addr 2
  ...
  prunedBlock=$(<"./selectedBlockNumber_0")
  blockNum=$(($prunedBlock - 0))
  addr=$(<"./contracts/address0")
  runScriptOnNode "./js/lookUpOldState.js" 0 $addr $blockNum
  runScriptOnNode "./js/lookUpOldState.js" 1 $addr $blockNum
